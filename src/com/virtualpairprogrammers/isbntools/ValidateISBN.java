...
public class ValidateISBN {
    ...
    public boolean checkISBN(String isbn) {
        ...
        if (isbn.length() == LONG_ISBN_LENGTH) {
            return IsThisAValidLongISBNNumber(isbn);
        } else if (isbn.length() == SHORT_ISBN_LENGTH) {
            return IsThisAValidShortISBNNumber(isbn);
        } throw new NumberFormatException("ISBN numbers must be 10 or 13 digits long");
    }

    private boolean IsThisAValidShortISBNNumber(String isbn) {
        int total = 0;
    ...
    private boolean IsThisAValidLongISBNNumber(String isbn) {
        int total = 0;
